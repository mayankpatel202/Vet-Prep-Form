# Vet Prep - Student Info Form
### Welcome to Vet Prep- we are so glad you're joining us! To help us learn a little bit more about you and your goals, please fill out this form ASAP (before class starts). We look forward to meeting you in the near future!

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: multiple-choice
* id: 26884ab7-db7c-408b-972c-4f52470c548d
* title: Immersive
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

Are you planning to join a Hack Reactor immersive? if so, when?

##### !end-question

##### !options

* I am not planning to join a Hack Reactor immersive program.
* I am hoping to join the next upcoming immersive and preparing for my TAA now.
* I would like to join Hack Reactor in the next 3-6 months.
* I would like to join Hack Reactor in 6-12 months.
* I am curious about coding, but I'm not sure when I'll join the Hack Reactor immersive, but I do want to join.

##### !end-options

##### !answer

* I am not planning to join a Hack Reactor immersive program.

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: short-answer
* id: ca3b47c8-bea9-44fc-9529-485c35602350
* title: Preferred Name
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

What is your preferred name?

##### !end-question

##### !placeholder

Preferred Name

##### !end-placeholder

##### !answer

Preferred Name

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: checkbox
* id: 44592085-e2c8-4a5a-a4dc-ae81cc4e15af
* title: Pronouns
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

What are your pronouns (which ones should we use to refer to you)?

##### !end-question

##### !options

* she/her
* he/him
* they/them
* ze/zir

##### !end-options

##### !answer

* she/her

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: multiple-choice
* id: ff4a491d-6a9d-4d8c-a574-2422fa2d6624
* title: Age
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

What is your age?

##### !end-question

##### !options

* 18-24
* 25-34
* 35-44
* 45-54
* 55+

##### !end-options

##### !answer

* 18-24

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: multiple-choice
* id: 30d285e2-f8fe-4ca1-ad81-a5d23267be6a
* title: Timezone
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

Which timezone are you planning on targeting for your Hack Reactor immersive experience?

##### !end-question

##### !options

* Remote Full-Time (Central) Immersive
* Remote Full-Time (Eastern) Immersive
* Remote Full-Time (Pacific) Immersive

##### !end-options

##### !answer

* Remote Full-Time (Central) Immersive

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: checkbox
* id: bac635a9-6095-4e89-9cc4-a8680c87b068
* title: Coding Experience
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

What prior experience do you have with JavaScript (JS) coding?

##### !end-question

##### !options

* I don't have prior coding experience.
* I am comfortable with basic JS syntax and can create variables and simple data structures.
* I can use JS to create conditional statements.
* I can use JS to iterate through arrays and objects.
* I can use JS to combine iteration and conditionals on nested arrays and objects.
* I have built a JS app from scratch.
* I have contributed to a JS app.
* I have used JS to contribute to a dynamic web page.

##### !end-options

##### !answer

* I don't have prior coding experience.

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: multiple-choice
* id: 173f650c-8d24-485d-9dea-99eb8d93092b
* title: TAA Attempts
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

How many times have you taken the TAA for admission to the immersive?

##### !end-question

##### !options

* 0
* 1
* 2
* 3+

##### !end-options

##### !answer

* 0

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: multiple-choice
* id: 804dd870-7141-439f-85e6-531131a3fc18
* title: Coding Timings
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

Which statement best describes your availability to practice code while enrolled in the Vet Prep experience?

##### !end-question

##### !options

* I plan to practice coding during Vet Prep hours only (9:30 am - 2:00 pm PT).
* I plan to dedicate a small chunk (1-5hrs) per week of time outside of Vet Prep hours coding.
* I plan to dedicate a significant amount (6-12hrs) per week of time outside of Vet Prep hours coding.
* I plan to dedicate my full attention (13+ hrs) outside of Vet Prep hours coding.

##### !end-options

##### !answer

* I plan to dedicate my full attention (13+ hrs) outside of Vet Prep hours coding.

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: multiple-choice
* id: 23eed02b-c38f-42bf-b4eb-b82189eac352
* title: Education
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

What is the highest degree or level of school you have completed?

##### !end-question

##### !options

* No schooling completed
* Some High School, no diploma
* High school graduate, diploma or the equivalent (for example: GED)
* Some college credit, no degree
* Trade/technical/vocational training
* Associate degree
* Bachelors degree
* Masters degree
* Professional degree
* Doctorate degree
* Prefer not to answer

##### !end-options

##### !answer

* Prefer not to answer

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: short-answer
* id: 26276d2c-7e12-4ef1-b221-1db0effc47e2
* title: Education more info
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

If applicable, what did you study?

##### !end-question

##### !placeholder

What did you study?

##### !end-placeholder

##### !answer

not applicable

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: multiple-choice
* id: 589eb8be-908e-4241-9dde-ff2a4f0498e8
* title: Marital Status
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

What is your marital status?

##### !end-question

##### !options

* Single, never married
* Married or domestic partnership
* Widowed
* Divorced
* Separated
* Prefer not to answer

##### !end-options

##### !answer

* Prefer not to answer

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: checkbox
* id: d9f2a64f-885d-4c8f-ae59-6192116bccc5
* title: Work Info
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

Are you currently...

##### !end-question

##### !options

* Employed for wages
* Self-employed
* Out of work and looking for work
* Out of work and not actively looking
* Homemaker
* Active student (this excludes this class)
* Military
* Retired

##### !end-options

##### !answer

* Retired

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: short-answer
* id: 7ebd6ca2-ab59-4a78-900b-051637f57a8c
* title: Career Path
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

Tell us more about your career path up until this point.  

##### !end-question

##### !placeholder

Career Path

##### !end-placeholder

##### !answer

Career Path

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: short-answer
* id: 56b3c03c-f530-4e77-a140-5792f2f43366
* title: Location Info
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

Where are you currently located?

##### !end-question

##### !placeholder

Currently Located

##### !end-placeholder

##### !answer

Currently Located

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: short-answer
* id: 584728b0-b5df-4b3c-9bea-7488976302e6
* title: Professional Goals
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

Have you set personal or professional goals for yourself recently?  If so, please share.  

##### !end-question

##### !placeholder

Goals

##### !end-placeholder

##### !answer

Goals

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: short-answer
* id: ab54bb48-5cca-41cf-8020-8bad2bd39e85
* title: Challenges
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

What challenges are you facing currently?

##### !end-question

##### !placeholder

Challenges

##### !end-placeholder

##### !answer

Challenges

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
<!-- Replace everything in square brackets [] and remove brackets  -->

### !challenge

* type: short-answer
* id: b52072cf-b86f-41ee-844e-be85bf4f54f4
* title: Serve Needs
<!-- * points: [1] (optional, the number of points for scoring as a checkpoint) -->
<!-- * topics: [python, pandas] (optional the topics for analyzing points) -->

##### !question

What can we do to better serve your needs?

##### !end-question

##### !placeholder

Your answer here...

##### !end-placeholder

##### !answer

Your answer here...

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: checkbox
* id: 362e71ca-695d-4704-a5fb-eb9933164285
* title: Programming Career

##### !question

How do you feel about starting a career in programming? (Check all boxes that apply) 

##### !end-question

##### !options

* Confident
* Excited
* Lucky
* Glad
* Courageous
* Impulsive
* Pleased
* Surprised
* At ease
* Certain
* Peaceful
* Devoted
* Bad
* Lousy
* Discouraged
* Hesitant
* Perplexed
* Unsure
* Skeptical
* Tense
* Pessimistic
* Fatigued
* Inferior
* Neutral
* Weary
* Terrified

##### !end-options

##### !answer

* Terrified

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: multiple-choice
* id: 11cd5dc2-e9e0-4873-b084-f14a46da7ca0
* title: Software Engineering Career

##### !question

How informed are you about software engineering as a career? 

##### !end-question

##### !options

* Very Informed
* Informed
* Uninformed
* I don't know anything about software engineering as a career

##### !end-options

##### !answer

* Very Informed

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: multiple-choice
* id: 1f5b258f-bdab-4c32-bc0d-38dfa0255bd0
* title: Method of Communication

##### !question

Generally speaking, what is your preferred method for communicating with a group providing a service to you?

##### !end-question

##### !options

* Phone Call
* Text Message
* Email
* Zoom/Skype/WebEx

##### !end-options

##### !answer

* Zoom/Skype/WebEx

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->

### !challenge

* type: paragraph
* id: 3288d8e6-ca9c-4231-9048-ce18a13e5c36
* title: Hack Reactor Prep Program

##### !question

What made you choose Hack Reactor's Vet Prep program?

##### !end-question

##### !placeholder

your answer here...

##### !end-placeholder
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: paragraph
* id: d303eaf8-9e2a-4ab9-8262-740f868ad7c0
* title: Competitor

##### !question

If you had to choose a competitor who would it be and why?

##### !end-question

##### !placeholder

your answer here...

##### !end-placeholder
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->



<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: short-answer
* id: dce5dcf1-7caf-4d26-ab4f-581d4cd6a634
* title: Learn New Information

##### !question

How do you learn new information for yourself or job?

##### !end-question

##### !placeholder

your answer here...

##### !end-placeholder

##### !answer

your answer here...

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: short-answer
* id: 1f8fb00f-5692-4819-936a-3c240ef0e182
* title: Reading Material

##### !question

Which publications or blogs do you read?

##### !end-question

##### !placeholder

your answer here...

##### !end-placeholder

##### !answer

your answer here...

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: short-answer
* id: 643ab755-38b4-4a73-b262-83b5f44099fd
* title: Social Networks
##### !question

Which associations and social networks do you participate in?

##### !end-question

##### !placeholder

your answer here...

##### !end-placeholder

##### !answer

your answer here...

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: paragraph
* id: e9a6ee2b-c1b3-45d8-b298-de8d3398d2de
* title: Life Experience
##### !question

What are you most eager to experience in life? 

##### !end-question

##### !placeholder

your answer here...

##### !end-placeholder
### !end-challenge

<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: paragraph
* id: 49eb39bd-f6c1-4fec-8b9a-c3fa45aa4310
* title: Experience with Hack Reactor

##### !question

What are you most eager to experience here with us? 

##### !end-question

##### !placeholder

your answer here...

##### !end-placeholder
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: checkbox
* id: 53eb81f9-82b3-4299-89c0-52d30ede0fd6
* title: Previous Interactions
##### !question

I have interacted with the following Hack Reactor individuals (Check any and all that apply).

##### !end-question

##### !options

* Enrollment Advisor
* Former Hack Reactor Graduate
* I have not interacted with any Hack Reactor staff or graduates
* Other

##### !end-options

##### !answer

* Enrollment Advisor

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->

### !challenge

* type: short-answer
* id: bdaf8c19-31e1-48e0-9dcb-b7086ce9f3ca
* title: Other referenced in previous question

##### !question

If you selected **Other** in the previous question please give more context.

##### !end-question

##### !placeholder

If Applicable

##### !end-placeholder

##### !answer

If Applicable

##### !end-answer
### !end-challenge

<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: multiple-choice
* id: b041e25c-7746-4eb2-8990-4aaaa948f8e2
* title: Application Submission

##### !question

Have you submitted your application to apply to our software engineering immersive?

##### !end-question

##### !options

* Yes
* No
* Unsure

##### !end-options

##### !answer

* Unsure

##### !end-answer
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: short-answer
* id: 1185ad03-34f6-42db-ba22-1d364b5e280e
* title: Enrollment Preventing

##### !question

If you get accepted into the immersive, what would prevent you from enrolling? 

##### !end-question

##### !placeholder

your answer here...

##### !end-placeholder

##### !answer

your answer here...

##### !end-answer
### !end-challenge

<!-- ======================= END CHALLENGE ======================= -->

<!-- >>>>>>>>>>>>>>>>>>>>>> BEGIN CHALLENGE >>>>>>>>>>>>>>>>>>>>>> -->
### !challenge

* type: paragraph
* id: edf1af64-7422-41b7-999d-b047f1c86c0a
* title: Any Concerns
##### !question

(Optional) If you have any other questions or concerns that we can address at this time, please enter them below:

##### !end-question

##### !placeholder

your answer here...

##### !end-placeholder
### !end-challenge
<!-- ======================= END CHALLENGE ======================= -->
